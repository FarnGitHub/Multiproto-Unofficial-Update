...
public class TranslationParityHelper {
    ...
    public static void replaceBefore(ProtocolVersion target, Block block, String key) {
        block.setTranslationKey((ProtocolVersionManager.isBefore(target) && Config.config.translationParity ? "multiproto." : "") + key);
    }

    public static void replaceBefore(ProtocolVersion target, Item item, String key) {
        item.setTranslationKey((ProtocolVersionManager.isBefore(target) && Config.config.translationParity ? "multiproto." : "") + key);
    }
}
